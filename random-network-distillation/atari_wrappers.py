...
def make_atari(env_id, max_episode_steps=4500, action_space=None):
    # from environments.gym_super_mario_bros import make
    from nes_py.wrappers import JoypadSpace
    from environments.gym_super_mario_bros.actions import RIGHT_ONLY, SIMPLE_MOVEMENT, COMPLEX_MOVEMENT
    from environments.mario_env import SuperMario_Env
    # env = gym.make(env_id)
    if action_space == 'RIGHT_ONLY':
        mario_action_space = RIGHT_ONLY
    elif action_space == 'SIMPLE_MOVEMENT':
        mario_action_space = SIMPLE_MOVEMENT
    elif action_space == 'COMPLEX_MOVEMENT':
        mario_action_space = COMPLEX_MOVEMENT
    elif action_space is None :
        mario_action_space = RIGHT_ONLY
    else:
        mario_action_space = RIGHT_ONLY

    env = SuperMario_Env(world=2, stage=1, scale=True)     #make('SuperMarioBros-2-1-v0')          # ('SuperMarioBros-v0')
    env = JoypadSpace(env, mario_action_space)
    ...
    # print(f'env id is {env_id}!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    if 'sparse' in env_id:
        # print(f'using sparse env!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        env = SparseRewardEnv(env)
...
class StickyActionEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        obs, reward, done, info = self.env.step(action)
        info.update({'taken_action': action})
        info.update({'rgb_obs': obs})
