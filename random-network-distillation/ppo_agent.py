...
class PpoAgent(object):
    ...
    def start_interaction(self, venvs, disable_policy_update=False):
        self.I = InteractionState(ob_space=self.ob_space, ac_space=self.ac_space,
            ...
            venvs=venvs, stochpol=self.stochpol, comm=self.comm_train)    # TODO: It's like a replay buffer, need to figure out the exact meaning.
    ...
    def collect_random_statistics(self, num_timesteps):
        ...
        for step in range(num_timesteps):
            for lump in range(self.I.nlump):
                ...
                ob, r, d, info = self.I.venvs[lump].step_wait()
                all_ob.append(ob)
                # ob: N x 84 x 84 x 4
                # action: N
                # reward: N
                # done: N
                # info: tuple of dict {'coins': 0, 'flag_get': False, 'life': 2, 'score': 0, 'stage': 1, 'status': 'small', 'time': 400, 'world': 1, 'x_pos': 40, 'y_pos': 93}
                # here N = 32
    ...
    def one_rollout(self, env, eps, save_image, e_greedy = 0):
        ob = env.reset()
        rollout = {'obs':[], 'action':[], 'reward':[], 'done':[], 'info':[]}
        rollout['obs'].append(ob)   # store s0
        done = False
        count = 0
        while not done:
            ob = np.array(ob)
            # ob = np.int8(ob*255.0)
            # ob_rgb = np.array(ob_rgb)
            if save_image:
                im = Image.fromarray(ob[:,:,-1])   # ob[:,:,-1]
                # import pdb; pdb.set_trace()
                im.save('./tmp_images/'+str(eps).zfill(4)+'/'+str(count).zfill(3)+".png")
            ob = ob[np.newaxis, ...]
            dict_obs = self.stochpol.ensure_observation_is_dict(ob)
            # action = [[1]]
            action = self.stochpol.get_action(dict_obs, update_obs_stats=self.update_ob_stats_every_step)
            if e_greedy:
                if random.random() < e_greedy:
                    action = [[random.randrange(env.action_space.n)]]
            ob, r, done, info = env.step(action[0][0])  # input action to env
            # env.render()
            rollout['action'].append(action[0][0])  # store a0 (a_i)
            rollout['reward'].append(r)             # store r0 (r_i)
            rollout['done'].append(done)            # store d0 (d_i)
            rollout['info'].append(info)            # store i0 (i_i)
            rollout['obs'].append(ob)               # store s1 (s_i+1)
            count += 1
        rollout['obs'] = np.stack(rollout['obs'])
        rollout['action'] = np.stack(rollout['action'])
        rollout['reward'] = np.stack(rollout['reward'])
        rollout['done'] = np.stack(rollout['done'])
        # rollout['info'] = np.stack(rollout['info'])
        return rollout


    def evaluate(self, env_id, seed, episodes, save_image, e_greedy=0):
        from cmd_util import make_eval_env
        env = make_eval_env(env_id, seed, wrapper_kwargs=dict(frame_stack=True))
        all_rollouts = []
        for i in range(episodes):
            print(i, 'th  episode')
            os.makedirs('./tmp_images/'+str(i).zfill(4)+'/', exist_ok=True)
            rollout = self.one_rollout(env, i, save_image, e_greedy)
            # import pdb; pdb.set_trace()
    ...
    def step(self):
        ...
        for l in range(self.I.nlump):
            obs, prevrews, news, infos = self.env_get(l)   # wait for info
            ...
            self.env_step(l, acs)   # input action to env
        ...
        if t == self.nsteps - 1 and not self.disable_policy_update:
            ...
            if not self.testing:  # TODO： this is the actually training....
    ...
    def restore_model(self, saver, load_dir, exp_name, mtype='latest'):
        saver.restore(tf.get_default_session(), os.path.join(load_dir, exp_name, mtype+'.ckpt'))
        print(f"loaded NN model from {load_dir}")

    def save_model(self, saver, save_dir, exp_name, mtype='latest'):
        os.makedirs(os.path.join(save_dir, exp_name), exist_ok=True)
        path_to_model = saver.save(tf.get_default_session(), os.path.join(save_dir, exp_name, mtype+'.ckpt'))
        print(f"saved model to {path_to_model}")
