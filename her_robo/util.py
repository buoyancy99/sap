...
class Normalizer():
    def __init__(self, shape, eps=1e-6):
        super().__init__()
        with self.scope:
            self.mean = tf.Variable(tf.zeros(shape), name='mean', trainable=False)
            self.std = tf.Variable(tf.ones(shape), name='std', trainable=False)
    ...
    def normalize(self, x, inverse=False):
        ...
        return tf.clip_by_value((x - self.mean) / self.std.maximum(self.eps), -5., 5.)
