        ...
        for progress, episode in enumerate(episodes):
            ...
            for i in range(known_len, epi_len):
                ...
                for j in range(min(plan_step, epi_len - i)):
                    ...
                    features[-1]['block_input'].append(float_feature(block_input.flatten()))
                    features[-1]['action_input'].append(int64_feature(action))
                    features[-1]['pos_input'].append(float_feature(pos_input))
                    features[-1]['step'].append(int64_feature([j]))
                    features[-1]['gt'].append(float_feature(normalize_v(gt, False)))
                    features[-1]['done'].append(float_feature([done]))

        for subseq in features:
            for key, val in subseq.items():
                subseq[key] = tf.train.FeatureList(feature=val)

            data = tf.train.SequenceExample(feature_lists=tf.train.FeatureLists(feature_list=subseq))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("serialize *.pkl rollouts to sub sequence tf records")
    parser.add_argument("--rollout_path", type=str, default="{}/Datasets/MarioRaw".format(os.getenv('DATASET_ROOT')))
    parser.add_argument("--savedir", type=str, default="{}/Datasets/MarioDynamics".format(os.getenv('DATASET_ROOT')))
    parser.add_argument("--n_worker", type=int, default=2)
    args = parser.parse_args()

    Rollout_To_TF(rollout_path=args.rollout_path, savedir=args.savedir + '{}'.format(plan_step), n_worker=args.n_worker)
