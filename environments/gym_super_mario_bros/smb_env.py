...
class SuperMarioBrosEnv(NESEnv):
    ...
    def _left_x_position(self):
        ...
        current_pos = self.ram[0x86].astype(np.int16)
        left_pos = self.ram[0x071c].astype(np.int16)

        return(current_pos - left_pos) % 256
    ...
    def _get_info(self):
        ...
        return dict(
            ...
            screen_x_pos=self._left_x_position
        )
    ...
    def backup(self):
        self._x_position_last_backup = self._x_position_last
        self._time_last_backup = self._time_last
        super()._backup()

    def restore(self):
        self._has_backup = True
        self._x_position_last = self._x_position_last_backup
        self._time_last = self._time_last_backup
        obs = super().reset()
        obs = obs.astype(np.float32) / 255.0
        return obs
