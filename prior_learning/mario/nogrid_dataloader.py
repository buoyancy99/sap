...
def mario_dataset(data_dir='{}/Datasets/MarioDemo'.format(os.getenv('DATASET_ROOT'))):
    ...
    feature_description = {
        'local_obses_no_mario': tf.io.FixedLenFeature([seq_len, 30, 30], tf.float32),
    ...
    def _parse_function(example_proto):
        ...
        return record['local_obses_no_mario'], record['action'], record['mask'], record['total_reward']


    dataset = tf.data.TFRecordDataset(filenames)
    dataset = dataset.map(_parse_function, num_parallel_calls=256)
