...
class imitation_model:
    def __init__(self, model_path='imitation/mario/ckpts', mode='eval'):
        self.model_path = os.path.join(model_path, 'imitation_{}_expert'.format(trained_on))
        self.mode = mode
    ...
    def make_model(self):
        if self.mode == 'train':
            dataset = imitation_dataset('{}/Datasets/MarioImitationExpert'.format(os.getenv('DATASET_ROOT')))
            obs_input, action_gt = dataset.get_next()
            obs_input = Input(tensor=obs_input)
        elif self.mode == 'eval':
            obs_input = Input(shape=[84, 84, 4])

        obs_feature = layers.Conv2D(16, 8, 4, padding='same', activation='relu')(obs_input) # 21
        obs_feature = layers.Conv2D(32, 4, 2, padding='same', activation='relu')(obs_feature)  # 11
        obs_feature = layers.Conv2D(64, 3, 2, padding='same', activation='relu')(obs_feature)  # 6
        obs_feature = layers.Conv2D(128, 3, 2, padding='same', activation='relu')(obs_feature)  # 3
        obs_feature = layers.GlobalMaxPool2D()(obs_feature)
        obs_feature = layers.Flatten()(obs_feature)
        obs_feature = layers.Dense(32, activation='relu')(obs_feature)
