...
class imitation_model:
    def __init__(self, model_path='imitation/mario/ckpts', mode='eval'):
        self.model_path = os.path.join(model_path, 'imitation_{}_expert_local'.format(trained_on))
        self.mode = mode
    ...
    def make_model(self):
        if self.mode == 'train':
            dataset = imitation_dataset_local('{}/Datasets/MarioImitationExpertLocal'.format(os.getenv('DATASET_ROOT')))
