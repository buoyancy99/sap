...
from imitation.mario.imitation_model_local import imitation_model
import os
import numpy as np
import cv2
...
experiment_name = 'imitation_local_world{}_stage{}_trained_on'.format(world, stage, trained_on)
...
def bench_imitation():
    ...
    for count in trange(1, 50001):
        _, action_dist = model.predict(info['local_obs'][:, :, :, None])
